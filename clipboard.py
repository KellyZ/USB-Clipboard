...
# WndProcType = ctypes.WINFUNCTYPE(c_int, wintypes.HWND, wintypes.UINT,
...
RegisterDeviceNotification.argtypes = [
    wintypes.HANDLE, wintypes.c_void_p, wintypes.DWORD]
...
class DEV_BROADCAST_VOLUMN(ctypes.Structure):
...
class WndProcHookMixin:
    ...
    def unregisterDeviceNotification(self, handle):
        if UnregisterDeviceNotification(handle) == 0:
            raise Exception(
                "Unable to unregister device notification messages")
...
if __name__ == "__main__":
    ...
    class MyFrame(wx.Frame, WndProcHookMixin):
        ...
        def __init__(self, parent):
            ...
            wx.Frame.__init__(
                self, parent, -1, "Insert and Remove USE Device and Watch STDOUT", size=(640, 480))

            self.Bind(wx.EVT_CLOSE, self.onClose)

            # Change the following guid to the GUID of the device you want
            # notifications for
            self.devNotifyHandle = self.registerDeviceNotification(
        ...
        def onDeviceChange(self, wParam, lParam):
            ...
            if lParam:
                ...
                if dbh.dbch_devicetype == DBT_DEVTYP_DEVICEINTERFACE:
                    ...
                    # Verify that the USB VID and PID match our assigned VID
                    # and PID
